...
const SellCard = ({price} : {price : number}) => {
    ...
    const onSubmit = (values: z.infer<typeof BuySchema>) => {
      ...
      setSuccess("") ;
      startTransition(() => {
        Sell(values,price,SymbolDetails.symbol)
        .then((data : any) => {
    ...
    return ( 
        <div className="flex flex-col justify-around h-full w-full mt-2">
            <div className="space-y-4">
                <h1 className="text-xl font-bold">You are looking to Sell : {SymbolDetails.symbol.symbol}</h1>
